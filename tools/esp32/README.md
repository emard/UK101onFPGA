...
where <filename> is optional and <offset> is optional starting address.
Also it can then execute machine code using LNK <start>. <offset> and <start>
are human-readable parameters visible as ASCII strings in hexdump,
See "5768" in this case:
    ...
    *
    ...
    00000130  20 20 3a cc 22 62 64 63  6f 64 65 22 3a c9 35 37  |  :."bdcode":.57|
...
offset 0x102 (0x20 byte) until the end of file.
    ...
    00000020  20 20 20 20 20 20 20 20  20 20 20 20 20 20 20 20  |                |
    *
    00000100  81 3f 20 31 35 38 37 32  00 30 30 30 30 24 24 24  |.? 15872.0000$$$|
    00000110  24 24 00 00 8c 1e 78 1e  82 1e 00 00 7b 1f 00 00  |$$....x.....{...|

So from 0x102 to the end of file we should write to ORAO 
RAM starting at 0x400 and then execute "LNK 5768"

Every program can have this procedure
slightly different. For boulder, this is the final procedure:
